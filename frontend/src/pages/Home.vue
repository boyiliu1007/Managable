<template>
  <div>
    ...
    <nav class="bg-gray-800 text-white flex justify-between items-center shadow-lg px-[2%] py-[1%] h-[10vh] mb-[2vh]">
      ...
      <button @click="logout" class="bg-red-500 hover:bg-red-600 rounded-md px-[1%] py-[0.5%]">
        Logout
      </button>
    </nav>
    ...
    <div class="h-[88vh] w-full px-[3%]">
      <div class="flex flex-col">
        <button @click="newTask" class="bg-red-500 hover:bg-red-600 rounded-md py-[0.5%] m-[1%] w-[10%] min-w-5 text-white text-lg">
          New Task
        </button>
    ...
    >
      ...
      >
        ...
        <button @click="closeModal" class="mt-4 bg-red-500 text-white px-4 py-2 rounded-md">
          Close
        </button>
      </div>
    </div>
...
const tasks = ref({
  // fake data
  "To Do": [
    { name: "Task A", dueDate: "2024-03-01", username: "Charlie" }
  ],
  "In Progress": [
